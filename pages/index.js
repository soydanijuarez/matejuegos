...
async function db({ table, method = "GET", body, filters = "", upsertOn = "" }) {
  try {
    ...
    if (method === "POST") h["Prefer"] = upsertOn
...
const THINK_MODALS = {
  ...
  orden: [
    ...
    "¿Dos números distintos pueden ocupar el mismo lugar en la recta numérica?",
  ],
  falta: [
    ...
    "¿Un número siempre tiene una sola división exacta posible?",
  ],
  valor: [
    "¿En 3.456, el dígito 3 representa miles?",
    "¿El mismo dígito puede valer distinto según su posición?",
    "¿Cuál es el número más grande que se puede escribir con los dígitos 1, 2, 3 y 4?",
  ],
};

// ─── PROBLEM GENERATORS ─────────────────────────────────────────────────────
function genCeros(lv) {
  const z = lv <= 2 ? rnd(1, 2) : rnd(2, 3);
  ...
  if (lv === 1 || lv === 3 || (lv === 4 && Math.random() > 0.5)) {
    ...
    return {
      ...
      feedbackWrong: `Revisá tu respuesta. Primero calculá ${base} × ${b}, después pensá en los ceros de ${a}.`,
      guides: [
        `¿Cuánto es ${base} × ${b}?`,
        `El ${a} tiene ${z} cero${z > 1 ? "s" : ""}. ¿Cómo afecta eso al resultado?`,
      ]
    };
  } else {
    ...
    return {
      ...
      feedbackWrong: `Revisá tu respuesta. Primero calculá ${q * d} ÷ ${d}, después pensá en los ceros.`,
      guides: [
        `¿Cuánto es ${q * d} ÷ ${d}?`,
        `El ${dividend} tiene ${z} cero${z > 1 ? "s" : ""}. ¿Cómo afecta eso al resultado?`,
      ]
    };
  }
}
...
function genOrden(lv) {
  ...
  let tries = 0;
  while (JSON.stringify(shuffled) === JSON.stringify(sorted) && tries < 20) {
    shuffled = [...nums].sort(() => Math.random() - 0.5); tries++;
  }
  return {
    type: "order", nums: shuffled, answer: [...sorted],
    displayAns: sorted.join(" < "),
    feedbackOk: `Correcto. El orden de menor a mayor es: ${sorted.join(" < ")}.`,
    feedbackWrong: "Revisá tu respuesta. Comparalos de izquierda a derecha, dígito por dígito.",
    guides: [
      `Fijate cuántas cifras tiene cada número. Más cifras = número más grande.`,
      `Si tienen la misma cantidad de cifras, comparalos de izquierda a derecha.`,
    ]
  };
}
...
function genFalta(lv) {
  ...
  if (lv <= 2) {
    ...
    return {
      ...
      feedbackOk: `Correcto. ${hideA ? `${a} × ${b} = ${result}` : `${a} × ${b} = ${result}`}.`,
      feedbackWrong: `Revisá tu respuesta. Pensá en la operación inversa: ¿cuánto es ${result} ÷ ${hideA ? b : a}?`,
      guides: [
        `La multiplicación y la división son operaciones inversas.`,
        `Si ___ × ${hideA ? b : a} = ${result}, entonces ${result} ÷ ${hideA ? b : a} = ___`,
      ]
    };
  } else {
    ...
    return {
      ...
      feedbackWrong: `Revisá tu respuesta. Usá la operación inversa para encontrar el número que falta.`,
      guides: [
        hideD ? `Si ${dividend} ÷ ___ = ${result}, calculá ${dividend} ÷ ${result}.`
               : `Si ___ ÷ ${d} = ${result}, calculá ${result} × ${d}.`,
        `Verificá tu respuesta usando la operación contraria.`,
      ]
    };
  }
}
...
function genValor(lv) {
  ...
  const POS = ["unidades de mil", "centenas", "decenas", "unidades"];

  if (lv === 1) {
    const p = rnd(0, 3);
    if (d[p] === 0) return genValor(1);
    return {
      type: "input",
      expr: `En el número ${num}, ¿cuántas ${POS[p]} hay?`,
      answer: String(d[p]),
      displayAns: `El ${num} tiene ${d[p]} ${POS[p]}.`,
      feedbackOk: `Correcto. El ${num} tiene ${d[p]} ${POS[p]}.`,
      feedbackWrong: `Revisá tu respuesta. El ${num} se lee: ${d[0]} unidades de mil, ${d[1]} centenas, ${d[2]} decenas y ${d[3]} unidades.`,
      guides: [
        `El número ${num} tiene 4 posiciones: unidades de mil, centenas, decenas, unidades.`,
        `Contá de derecha a izquierda: unidades (${d[3]}), decenas (${d[2]}), centenas (${d[1]}), unidades de mil (${d[0]}).`,
      ]
    };
  }
  ...
  if (lv === 2) {
    ...
    return {
      ...
      placeholder: `Ejemplo: ${labels[0]}+...`,
      answer,
      displayAns: `${num} = ${labels.join(" + ")}`,
      feedbackOk: `Correcto. ${num} = ${labels.join(" + ")}.`,
      feedbackWrong: `Revisá tu respuesta. Escribí el valor de cada parte separado con +, sin espacios.`,
      guides: [
        `El ${num} tiene: ${d[0]} unidades de mil = ${d[0] * 1000}, ${d[1]} centenas = ${d[1] * 100}...`,
        `Escribí cada parte y unilas con + sin espacios. Ejemplo: ${labels[0]}+${labels[1] || labels[0]}.`,
      ]
    };
  }
  ...
  if (lv === 3) {
    ...
    return {
      ...
      feedbackWrong: `Revisá tu respuesta. Sumá cada parte: ${d[0] * 1000} + ${d[1] * 100} + ${d[2] * 10} + ${d[3]}.`,
      guides: [
        `Convertí cada parte a número: ${d[0]} unidades de mil = ${d[0] * 1000}.`,
        `Sumá todas las partes de mayor a menor.`,
      ]
    };
  }
  ...
  return {
    ...
    expr: `${a}  ___  ${b}`,
    answer,
    displayAns: `${a} ${answer} ${b}.`,
    feedbackOk: `Correcto. ${a} ${answer} ${b}.`,
    feedbackWrong: `Revisá tu respuesta. Comparalos posición por posición, de izquierda a derecha.`,
    guides: [
      `Compará el primer dígito de cada número (unidades de mil).`,
      `Si el primer dígito es igual, pasá al segundo (centenas). ¿Cuál es mayor?`,
    ]
  };
}
...
const styles = {
  page: (bg = "#F8FAFC") => ({
    ...
    background: bg, padding: 20, fontFamily: "'Inter', 'Nunito', system-ui, sans-serif",
  }),
  card: {
    background: T.white, borderRadius: T.radius, padding: "36px 32px",
  ...
  input: {
    ...
    border: `1.5px solid ${T.border}`, fontSize: 16, fontFamily: "inherit",
    outline: "none", boxSizing: "border-box", marginBottom: 12,
    color: T.text, background: T.white, transition: "border-color 0.15s",
  },
  btn: (color = "#1E293B", variant = "filled", extra = {}) => ({
    padding: "11px 20px", borderRadius: T.radiusSm, fontSize: 14, fontWeight: 600,
...
function ThinkModal({ text, onClose }) {
  return (
    <div style={{
      ...
      <div style={{
        background: T.white, borderRadius: T.radius, padding: "32px 28px",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div style={{
            ...
            padding: "4px 10px", fontSize: 12, fontWeight: 600, letterSpacing: 0.5,
          }}>Para pensar</div>
        ...
        <p style={{ fontSize: 18, fontWeight: 600, color: T.text, lineHeight: 1.5, margin: "0 0 24px" }}>
          {text}
        </p>
        <button onClick={onClose} style={{ ...styles.btn("#1E293B", "filled", { width: "100%" }) }}>
    ...
    <div style={styles.page()}>
      <div style={{ ...styles.card, textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <button onClick={() => go("teacher-login")} style={styles.btn("#334155", "filled", { width: "100%", padding: "14px", fontSize: 15 })}>
            Panel docente
          </button>
          <button onClick={() => go("student-enter")} style={styles.btn("#3B82F6", "filled", { width: "100%", padding: "14px", fontSize: 15 })}>
...
function TeacherLogin({ go, setTeacher }) {
  ...
  const login = () => {
    if (code.trim().toUpperCase() === TEACHER_CODE) { setTeacher({}); go("teacher-dashboard"); }
  ...
  return (
    <div style={styles.page()}>
      <div style={styles.card}>
        ...
        <input style={styles.input} placeholder="Código de acceso" value={code} maxLength={12} type="password"
          onChange={e => setCode(e.target.value.toUpperCase())} onKeyDown={e => e.key === "Enter" && login()} />
        {err && <p style={{ color: T.error, fontSize: 13, margin: "0 0 12px" }}>{err}</p>}
        <button style={styles.btn("#334155", "filled", { width: "100%" })} onClick={login}>Ingresar</button>
      </div>
    </div>
  );
}
...
function TeacherDashboard({ go }) {
  ...
  const createClass = async () => {
    ...
    // Enable all games by default
    if (Array.isArray(cls) && cls[0]) {
      const code = cls[0].code;
      await Promise.all(GAMES.map(g => db({ table: "game_assignments", method: "POST", body: { class_code: code, game_id: g.id, enabled: true } })));
    }
    setNewName(""); await loadClasses(); setCreating(false);
  };

  const toggleGame = async (gameId, currentlyEnabled) => {
    await db({ table: "game_assignments", method: "POST", upsertOn: "class_code,game_id", body: { class_code: selected.code, game_id: gameId, enabled: !currentlyEnabled } });
    setAssignments(prev => {
      const exists = prev.find(a => a.game_id === gameId);
      if (exists) return prev.map(a => a.game_id === gameId ? { ...a, enabled: !currentlyEnabled } : a);
      return [...prev, { game_id: gameId, enabled: !currentlyEnabled }];
    });
  };

  const isEnabled = (gameId) => {
    const a = assignments.find(a => a.game_id === gameId);
  ...
  const headerStyle = { background: "#1E293B", color: "white", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" };

  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={headerStyle}>
        <div>
          <div style={{ fontSize: 11, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 1, fontWeight: 600 }}>Panel docente</div>
          <div style={{ fontSize: 18, fontWeight: 700 }}>MateJuegos</div>
        </div>
        <button onClick={() => go("home")} style={styles.btn("#334155", "filled", { fontSize: 13 })}>Salir</button>
      </div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 16px", display: "grid", gridTemplateColumns: "260px 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: T.white, borderRadius: T.radius, padding: 20, border: `1px solid ${T.border}` }}>
            ...
            <input style={{ ...styles.input, marginBottom: 10 }} placeholder="Nombre (ej: 4to A)" value={newName}
              onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && createClass()} />
            <button onClick={createClass} disabled={creating || !newName.trim()}
              style={styles.btn("#334155", "filled", { width: "100%", opacity: creating || !newName.trim() ? 0.5 : 1 })}>
          ...
          <div style={{ background: T.white, borderRadius: T.radius, padding: 20, border: `1px solid ${T.border}` }}>
            ...
            {classes.map(cls => (
              <div key={cls.id} onClick={() => selectClass(cls)} style={{
                border: `1.5px solid ${selected?.id === cls.id ? "#3B82F6" : T.border}`,
                borderRadius: T.radiusSm, padding: "12px 14px", marginBottom: 8, cursor: "pointer",
                background: selected?.id === cls.id ? "#EFF6FF" : T.white, transition: "all 0.15s",
              }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ background: "#EFF6FF", color: "#1D4ED8", borderRadius: 6, padding: "2px 8px", fontWeight: 700, fontSize: 12, letterSpacing: 1 }}>{cls.code}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          {!selected && (
            <div style={{ background: T.white, borderRadius: T.radius, padding: 48, textAlign: "center", border: `1px solid ${T.border}` }}>
              <p style={{ color: T.hint, fontWeight: 500, margin: 0 }}>Seleccioná una clase para ver los datos</p>
            </div>
          )}
          {selected && (
            <div style={{ background: T.white, borderRadius: T.radius, padding: 24, border: `1px solid ${T.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
                <div>
                  <h3 style={{ margin: "0 0 4px", color: T.text, fontWeight: 700 }}>{selected.name}</h3>
                  <p style={{ margin: 0, color: T.hint, fontSize: 12 }}>Código: <strong style={{ color: "#1D4ED8" }}>{selected.code}</strong></p>
                </div>
                <button onClick={() => selectClass(selected)} style={styles.btn("#F1F5F9", "filled", { color: T.muted, fontSize: 12 })}>Actualizar</button>
              </div>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 20, borderBottom: `1px solid ${T.border}`, paddingBottom: 0 }}>
                {["results", "games", "attempts", "reflections"].map(t => (
                  <button key={t} onClick={() => setTab(t)} style={{
                    ...styles.btn("transparent", "filled", {
                      color: tab === t ? "#3B82F6" : T.muted, borderRadius: "8px 8px 0 0",
                      borderBottom: tab === t ? "2px solid #3B82F6" : "2px solid transparent",
                      padding: "8px 14px", fontSize: 13,
                    })
                  }}>
                    {t === "results" ? "Resultados" : t === "games" ? "Juegos activos" : t === "attempts" ? "Intentos" : "Reflexiones"}
                    {t === "reflections" && reflections.length > 0 && (
                      <span style={{ background: "#EF4444", color: "white", borderRadius: 99, padding: "1px 6px", fontSize: 10, marginLeft: 4 }}>{reflections.length}</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              {!loadingData && tab === "results" && (
                <>
                  {students.length === 0 && <p style={{ color: T.hint, textAlign: "center", padding: "32px 0" }}>Sin datos todavía. Código de clase: <strong style={{ color: "#1D4ED8" }}>{selected.code}</strong></p>}
                  {students.map(name => (
                            ...
                            <div key={g.id} style={{ background: T.white, borderRadius: T.radiusSm, padding: "10px 12px", border: `1px solid ${T.border}` }}>
                              ...
                              {r ? (
                                <div style={{ display: "flex", gap: 10, fontSize: 12 }}>
                                  <span style={{ color: "#2563EB", fontWeight: 700 }}>{r.score} pts</span>
                                  <span style={{ color: T.success, fontWeight: 700 }}>{r.correct} ✓</span>
                                  <span style={{ color: T.error, fontWeight: 700 }}>{r.wrong} ✗</span>
                                </div>
                              ) : <div style={{ color: T.hint, fontSize: 11 }}>Sin actividad</div>}
                            </div>
                          );
                        })}
                      </div>
              ...
              {!loadingData && tab === "games" && (
                <div>
                  <p style={{ color: T.muted, fontSize: 13, margin: "0 0 16px" }}>Activá o desactivá juegos para esta clase.</p>
                  {GAMES.map(g => {
                    const enabled = isEnabled(g.id);
                    return (
                      <div key={g.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", borderRadius: T.radiusSm, border: `1px solid ${T.border}`, marginBottom: 8, background: T.white }}>
                        <div>
                          <div style={{ fontWeight: 600, color: T.text, fontSize: 14 }}>{g.name}</div>
                          <div style={{ color: T.hint, fontSize: 12, marginTop: 2 }}>{g.desc}</div>
                        </div>
                        <button onClick={() => toggleGame(g.id, enabled)} style={{
                          ...styles.btn(enabled ? T.success : T.hint, "filled", { fontSize: 13, minWidth: 80 })
                        }}>
              ...
              {!loadingData && tab === "attempts" && (
                <>
                  {Object.keys(attemptStats).length === 0 && <p style={{ color: T.hint, textAlign: "center", padding: "32px 0" }}>Sin datos de intentos todavía.</p>}
                  {Object.values(attemptStats).map((s, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderRadius: T.radiusSm, border: `1px solid ${T.border}`, marginBottom: 8, background: T.white }}>
                        <div>
                          <div style={{ fontWeight: 600, color: T.text }}>{s.student}</div>
                          <div style={{ fontSize: 12, color: T.hint, marginTop: 2 }}>{game?.name}</div>
                        </div>
                  ...
                  {reflections.map((r, i) => (
                    <div key={i} style={{ background: "#F5F3FF", borderRadius: T.radiusSm, padding: 16, marginBottom: 10, borderLeft: "3px solid #8B5CF6" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                        <strong style={{ color: T.text, fontSize: 14 }}>{r.student_name}</strong>
                        <span style={{ color: T.hint, fontSize: 11 }}>{new Date(r.created_at).toLocaleDateString("es-AR")}</span>
                      </div>
...
function StudentEnter({ go, setStudent }) {
  ...
  const enter = async () => {
    ...
    if (Array.isArray(d) && d.length > 0) {
      setStudent({ name: name.trim(), classCode: code.toUpperCase().trim(), className: d[0].name });
      go("game-menu");
    } else setErr("Código no encontrado. Pedíselo a tu profe.");
    setLoading(false);
  };
  return (
    <div style={styles.page("#EFF6FF")}>
      <div style={styles.card}>
        <button onClick={() => go("home")} style={{ background: "none", border: "none", color: T.hint, cursor: "pointer", fontSize: 13, marginBottom: 24, padding: 0 }}>← Volver</button>
        ...
        <input style={styles.input} placeholder="Ej: Sofía" value={name} onChange={e => setName(e.target.value)} />
        <label style={{ fontSize: 13, fontWeight: 600, color: T.text, display: "block", marginBottom: 6 }}>Código de clase</label>
        <input style={{ ...styles.input, textTransform: "uppercase", letterSpacing: 4, fontSize: 20, fontWeight: 700, textAlign: "center" }}
          placeholder="ABC123" maxLength={6} value={code}
          onChange={e => setCode(e.target.value.toUpperCase())}
          onKeyDown={e => e.key === "Enter" && enter()} />
        {err && <p style={{ color: T.error, fontSize: 13, margin: "0 0 12px" }}>{err}</p>}
        <button style={styles.btn("#3B82F6", "filled", { width: "100%", padding: "13px", fontSize: 15 })} onClick={enter} disabled={loading}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: "#1E293B", color: "white", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 11, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 1, fontWeight: 600 }}>{student.className}</div>
          <div style={{ fontSize: 18, fontWeight: 700 }}>Hola, {student.name}</div>
        </div>
        <button onClick={() => go("home")} style={styles.btn("#334155", "filled", { fontSize: 13 })}>Salir</button>
      </div>
      ...
      {!loading && (
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 16px" }}>
          ...
          {enabledGames.length === 0 && <p style={{ color: T.hint }}>Tu docente todavía no activó ningún juego. Consultá con ella.</p>}
                    ...
                    return (
                      <button key={lv} onClick={() => !isLocked && startGame(game.id, lv)}
                        style={{
                          ...
                          borderRadius: T.radiusSm, border: `1px solid ${isLocked ? T.border : isDone ? "#D1FAE5" : game.color}`,
                          background: isLocked ? T.bg : isDone ? "#F0FDF4" : `${game.color}12`,
                          cursor: isLocked ? "not-allowed" : "pointer", fontFamily: "inherit", transition: "all 0.15s",
                        }}>
                        <span style={{ fontSize: 14, minWidth: 18 }}>{isLocked ? "🔒" : isDone ? "✓" : "▶"}</span>
...
function GamePlayer({ student, gameSession, go }) {
  ...
  const [thinkModal, setThinkModal]   = useState(null);
  ...
  const newProb = () => {
    ...
    setGuideStep(0); setGuideAns(""); setReflectAns(""); setReflectSaved(false);
    setShowReflect(false); setHintsUsed(0); setOrderSelected([]);
  ...
  const checkAnswer = (val) => {
    ...
    if (ok) {
      ...
      setCurrentFeedback(prob.feedbackOk);
      ...
      if (showThink) setThinkModal(pick(THINK_MODALS[gameId]));
      setScore(s => s + 10); setCorrect(c => c + 1);
      setCil(c => c + 1);
      setPhase("correct");
    } else {
      setLastAnswer(prob.type === "order" ? [...orderSelected] : answer);
      setCurrentFeedback(prob.feedbackWrong);
      setWrong(c => c + 1);
      setPhase("wrong");
      setOrderSelected([]);
    }
  ...
  const handleContinue = async () => {
    if (cil >= 5) {
      ...
      await Promise.all([
        ...
        db({ table: "student_progress", method: "POST", upsertOn: "class_code,student_name,game_id", body: { class_code: student.classCode, student_name: student.name, game_id: gameId, level_reached: nextLevel, exercises_completed: totalEx.current, completed: level >= 4, last_played: new Date().toISOString() } }),
      ]);
  ...
  const handleOrderClick = (num) => {
    if (orderSelected.includes(num)) setOrderSelected(orderSelected.filter(n => n !== num));
    else setOrderSelected([...orderSelected, num]);
  };
  ...
  if (finished) return (
    <div style={styles.page()}>
      {thinkModal && <ThinkModal text={thinkModal} onClose={() => setThinkModal(null)} />}
      <div style={{ ...styles.card, textAlign: "center" }}>
        ...
        <button style={styles.btn("#334155", "filled", { width: "100%" })} onClick={() => go("game-menu")}>Volver a los juegos</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: `${game.color}08`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <button onClick={saveAndExit} style={styles.btn("#F1F5F9", "filled", { color: T.muted, fontSize: 13 })}>← Volver al menú</button>
      ...
      {/* Card */}
      <div style={{ ...styles.card, maxWidth: 480, animation: "fadeUp 0.2s ease" }}>

        {/* QUESTION */}
        {phase === "question" && (
          <>
            <p style={{ textAlign: "center", color: T.hint, fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 20 }}>
              {prob.type === "order" ? "Ordená de menor a mayor" : prob.type === "choice" ? "Elegí el símbolo correcto" : "Resolvé"}
            </p>

            {prob.type === "input" && (
              <>
                <div style={{ textAlign: "center", marginBottom: 20 }}>
                  <p style={{ fontSize: prob.expr.length > 40 ? 16 : 26, fontWeight: 700, color: T.text, margin: 0, lineHeight: 1.5 }}>
                    {prob.expr.includes("___") ? prob.expr : `${prob.expr} =`}
                  </p>
                  {!prob.expr.includes("___") && (
                    <div style={{ fontSize: 28, fontWeight: 700, color: game.color, borderBottom: `2px solid ${game.color}`, paddingBottom: 2, minWidth: 100, display: "inline-block", marginTop: 8 }}>
                      {answer || "\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0"}
                    </div>
                  )}
                </div>
                <input ref={inputRef} type="text" inputMode={prob.flexible ? "text" : "numeric"}
                  placeholder={prob.placeholder || (prob.flexible ? "Escribí la descomposición..." : "Escribí el resultado")}
                  value={answer}
                ...
                <button onClick={() => checkAnswer()} disabled={!answer}
                  style={styles.btn(answer ? game.color : "#E2E8F0", "filled", { width: "100%", color: answer ? "white" : T.hint, cursor: answer ? "pointer" : "not-allowed" })}>
                  Comprobar
                </button>
              </>
            )}
            ...
            {prob.type === "choice" && (
              <>
                <p style={{ fontSize: 30, fontWeight: 700, color: T.text, textAlign: "center", marginBottom: 24 }}>{prob.expr}</p>
                <div style={{ display: "flex", gap: 12 }}>
                  {prob.choices.map(c => (
                    <button key={c} onClick={() => checkAnswer(c)}
                      style={styles.btn("#F1F5F9", "filled", { flex: 1, fontSize: 30, color: T.text, padding: "20px" })}>
                      {c}
                    </button>
                  ))}
                </div>
            ...
            {prob.type === "order" && (
              <>
                  ...
                  {prob.nums.map((n, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => handleOrderClick(n)}
                        style={styles.btn(sel ? game.color : "#F1F5F9", "filled", { color: sel ? "white" : T.text, padding: "12px 16px", fontSize: 18, fontWeight: 700, minWidth: 80, position: "relative" })}>
                        {n}
                ...
                {orderSelected.length > 0 && (
                  <div style={{ background: T.bg, borderRadius: T.radiusSm, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: T.muted }}>
                    Tu orden: {orderSelected.join(" → ")}
                  </div>
                )}
                <div style={{ display: "flex", gap: 8 }}>
                  {orderSelected.length > 0 && (
                    <button onClick={() => setOrderSelected([])}
                      style={styles.btn("#F1F5F9", "filled", { flex: 1, color: T.muted, fontSize: 13 })}>Borrar</button>
                  )}
                  <button onClick={() => checkAnswer(orderSelected)} disabled={orderSelected.length !== prob.nums.length}
                    style={styles.btn(orderSelected.length === prob.nums.length ? game.color : "#E2E8F0", "filled", {
                      flex: 2, color: orderSelected.length === prob.nums.length ? "white" : T.hint,
                      cursor: orderSelected.length === prob.nums.length ? "pointer" : "not-allowed",
                    })}>
                    Confirmar orden
                  </button>
                </div>
        ...
        {phase === "wrong" && (
          <div style={{ animation: "fadeUp 0.2s ease" }}>
            <div style={{ background: T.errorBg, border: `1px solid #FECACA`, borderRadius: T.radiusSm, padding: "14px 16px", marginBottom: 20 }}>
              <p style={{ fontSize: 14, color: T.error, fontWeight: 600, margin: 0, lineHeight: 1.5 }}>{currentFeedback}</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button onClick={() => { setHintsUsed(h => h + 1); setPhase("guide"); setGuideStep(0); setGuideAns(""); }}
                style={styles.btn("#334155", "filled", { width: "100%" })}>Ver guía</button>
              <button onClick={() => { setAnswer(""); setOrderSelected([]); setPhase("question"); }}
                style={styles.btn("#F1F5F9", "filled", { width: "100%", color: T.text })}>Intentar nuevamente</button>
              <button onClick={() => {
                ...
                setPhase("question");
              }} style={styles.btn("transparent", "filled", { width: "100%", color: T.muted, fontSize: 13 })}>
          ...
          <div style={{ animation: "fadeUp 0.2s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: T.hint, textTransform: "uppercase", letterSpacing: 0.5 }}>
                Guía ({guideStep + 1} de {prob.guides.length})
              </span>
            </div>
            <div style={{ background: "#EFF6FF", border: `1px solid #BFDBFE`, borderRadius: T.radiusSm, padding: "14px 16px", marginBottom: 16 }}>
              <p style={{ fontSize: 15, color: "#1E40AF", margin: 0, lineHeight: 1.5, fontWeight: 500 }}>{prob.guides[guideStep]}</p>
            </div>
            <input ref={inputRef} type="text" placeholder="Escribí lo que pensás..."
              value={guideAns} onChange={e => setGuideAns(e.target.value)}
              style={{ ...styles.input, fontSize: 15, textAlign: "center" }}
              onKeyDown={e => {
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setAnswer(""); setOrderSelected([]); setPhase("question"); }}
                style={styles.btn("#F1F5F9", "filled", { flex: 1, color: T.muted })}>Salir</button>
              <button onClick={() => {
                if (!guideAns.trim()) return;
                if (guideStep < prob.guides.length - 1) { setGuideStep(g => g + 1); setGuideAns(""); }
                else { setAnswer(""); setOrderSelected([]); setPhase("question"); }
              }} style={styles.btn("#334155", "filled", { flex: 2 })}>
                {guideStep < prob.guides.length - 1 ? "Siguiente" : "Intentar nuevamente"}
              </button>
            </div>
          </div>
        )}
        ...
        {phase === "correct" && (
          <div style={{ animation: "fadeUp 0.2s ease" }}>
            <div style={{ background: T.successBg, border: `1px solid #BBF7D0`, borderRadius: T.radiusSm, padding: "14px 16px", marginBottom: 20 }}>
              <p style={{ fontSize: 14, color: T.success, fontWeight: 600, margin: 0, lineHeight: 1.5 }}>{currentFeedback}</p>
            </div>
              ...
              <div style={{ background: "#F5F3FF", border: `1px solid #DDD6FE`, borderRadius: T.radiusSm, padding: "16px", marginBottom: 16 }}>
                ...
                {!reflectSaved ? (
                  <>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {reflectAns.trim() && <button onClick={saveReflection} style={styles.btn(T.success, "filled", { flex: 1, fontSize: 13 })}>Guardar</button>}
                      <button onClick={handleContinue} style={styles.btn("#F1F5F9", "filled", { flex: 1, color: T.muted, fontSize: 13 })}>Omitir</button>
                    </div>
                  </>
                ) : (
                  <>
                    <p style={{ color: T.success, fontSize: 13, fontWeight: 600, margin: "0 0 12px" }}>Respuesta guardada. Tu docente la va a ver después.</p>
                    <button onClick={handleContinue} style={styles.btn("#334155", "filled", { width: "100%" })}>Continuar</button>
                  </>
                )}
              </div>
            )}
            ...
            {!showReflect && (
              <button onClick={handleContinue} style={styles.btn(game.color, "filled", { width: "100%", fontSize: 15 })}>
